...
- `vms/<name>/` — one directory per microVM (the VM analogue of `machines/`), discovered by `flake-parts/vms.nix`. Each becomes a **standalone** `nixosConfigurations.<name>` built with the microvm *guest* module + the injected hub + the guest baseline (`flake-parts/vm-guest-base.nix`), plus a `packages.<system>.microvm-<name>` runner. A `vms/<name>/default.nix` is thin like a machine file: it sets `ft.*` (a docker VM enables `ft.containers` + `ft.komodo`), resources, and its tap interface. System is read from `vms/<name>/var/system`, else `x86_64-linux`. A host runs a VM by reference (`ft.microvms` → `microvm.vms.<name>.flake = self`) so the guest is never evaluated inside the host. VM names must not collide with machine names.
