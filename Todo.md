...
- [x] **Module docs custom formatter** — replace `nixosOptionsDoc`'s flat CommonMark output with a custom renderer that groups `ft.*` options by feature (`##` per feature), emits a table of contents, and adds one-line feature summaries; source from `optionsJSON` rather than `optionsCommonMark`
- [x] **Streamline `ft.*` option descriptions** — many are dense and implementation-heavy (internals, edge cases, cross-references assumed to already be familiar), which reads rough now that the grouped README puts them front and center. Rewrite for a skimming reader: plain language first, technical specifics (requirements, defaults, gotchas) kept but secondary, without losing accuracy.
