...
}:
{
  ...
  options.ft.sops = {
    useTPM = lib.mkEnableOption "TPM2 for decryption via age-plugin-tpm" // {
      description = "Adds age-plugin-tpm, enables the TPM2 subsystem, and configures sops to read the age identity from /var/lib/sops-nix/key.txt.";
    };
    useYubikey = lib.mkEnableOption "Yubikey for decryption via age-plugin-yubikey" // {
      description = "Adds age-plugin-yubikey, starts pcscd, and configures sops to read the age identity stub from /var/lib/sops-nix/key.txt.";
    };
  };
